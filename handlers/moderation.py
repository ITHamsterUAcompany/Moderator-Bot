...
async def announce_admin_recruitment(message: Message, bot: Bot):
    # доступ тільки для старшого складу
    if message.from_user.id not in SENIOR_ADMINS:
        await message.reply("⛔ Ця команда доступна лише старшому складу адміністрації.")
        return

    image_path = "handlers/banner.jpg" # або інший шлях
    ...
    text = (
        ...
        "готових допомагати підтримувати порядок у спільноті.\n\n"
        "🔹 Вимоги:\n"
        "• Активність у чаті\n"
        "• Ввічливість і відповідальність\n"
        "• Бажання допомагати іншим\n\n"
        "👇 Обери дію нижче:"
    )

    try:
        # якщо файл існує — відправляємо фото (відкриваємо як file-like)
        if os.path.exists(image_path):
            ...
            # fallback: якщо немає картинки — просто відправляємо текст + кнопки
        ...
        await message.reply(f"❌ Помилка при відправці оголошення: {type(e).__name__}: {e}")
