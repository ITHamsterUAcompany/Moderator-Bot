import json
import os
from dataclasses import dataclass, asdict
from datetime import datetime
from typing import Any, Dict, List

from config import settings

@dataclass
class HistoryEntry:
    type: str
    reason: str
    date: str
    until: str = None
    text: str = None

class DataStore:
    """JSON-backed store with live reload support."""

    def __init__(self, file_path: str = settings.DATA_FILE):
        self.file_path = file_path
        self.data: Dict[str, Any] = {
            "muted_users": {},
            "warnings": {},
            "karma": {},
            "history": {},
            "banned_users": [],
        }
        self._load_from_disk()

    # ---------------- Disk I/O ----------------
    def _load_from_disk(self) -> None:
        """Reloads data from JSON file if exists."""
        if os.path.exists(self.file_path):
            try:
                with open(self.file_path, "r", encoding="utf-8") as f:
                    loaded = json.load(f)
                    if isinstance(loaded, dict):
                        self.data.update(loaded)
            except Exception as e:
                print(f"[DataStore] Load error: {e}")

    def save(self) -> None:
        """Writes current data to JSON file."""
        with open(self.file_path, "w", encoding="utf-8") as f:
            json.dump(self.data, f, ensure_ascii=False, indent=4)

    def refresh(self) -> None:
        """Force reload from disk (use before reading)."""
        self._load_from_disk()

    # ---------------- History ----------------
    def append_history(self, user_id: int, entry: HistoryEntry) -> None:
        uid = str(user_id)
        self.refresh()  # ✅ ensure we're appending to the latest version
        self.data.setdefault("history", {}).setdefault(uid, []).append(asdict(entry))
        self.save()

    def get_history(self, user_id: int) -> List[HistoryEntry]:
        self.refresh()  # ✅ always load latest version from disk
        raw = self.data.get("history", {}).get(str(user_id), [])
        return [HistoryEntry(**item) for item in raw]

    # ---------------- Karma ----------------
    def get_karma(self, user_id: int, is_admin: bool = False) -> int:
        uid = str(user_id)
        self.refresh()
        if uid not in self.data.get("karma", {}):
            self.data.setdefault("karma", {})[uid] = 1000 if is_admin else 0
            self.save()
        return int(self.data["karma"][uid])

    def set_karma(self, user_id: int, value: int) -> int:
        uid = str(user_id)
        self.refresh()
        self.data.setdefault("karma", {})[uid] = value
        self.save()
        return value

    def add_karma(self, user_id: int, delta: int) -> int:
        current = self.get_karma(user_id)
        new_val = max(-1000, min(1000, current + delta))
        return self.set_karma(user_id, new_val)

# ✅ Глобальний об’єкт
store = DataStore()
